...
// Find all TypeScript files in your bots directory (excluding test files)
const entryPoints = fastGlob.sync('./src/bots/**/*.ts').filter((file) => !file.endsWith('test.ts'));

// Dependencies from bot-layer are available in Lambda runtime, so mark them as external
const botLayerDeps = [...Object.keys(botLayer.dependencies), '@aws-sdk/client-*'];

// Define the esbuild options
const esbuildOptions = {
  entryPoints: entryPoints,
  bundle: true, // Bundle imported functions (including phenoml SDK)
