...
/**
 * A Medplum Bot that generates International Patient Summary (IPS) using PhenoML.
 *
...
// Resource types to include in IPS Bundle
...
export async function handler(
  ...
  try {
    ...
    if (inputBundle) {
      ...
      // Fetch patient data from Medplum and build bundle
    ...
    const result = await client.summary.create({
      mode: 'ips',
      fhir_resources: fhirBundle as phenoml.summary.FhirBundle,
    });
 ...
 */
async function buildPatientBundle(
  medplum: MedplumClient,
  patientId: string
): Promise<Bundle> {
  // Fetch the patient
  ...
  for (const resourceType of IPS_RESOURCE_TYPES) {
    try {
      ...
      // Some resource types may not exist or have different search params
